...
x, y = 0,0
board = np.zeros((20, 20), dtype=str)
    ...
    for row in board:
        ...
        for letter in row:
            if letter in word.word and letter != "":
                ...
                if 0 <= proposed_x + word.orientation[0] * (word.size - 1) < 20 and 0 <= proposed_y + word.orientation[1] * (word.size - 1) < 20:
...
def check_placement(board, word, x, y, coords_skip=None):
    ...
    for letter in word.word:
        if not (0 <= x < 20 and 0 <= y < 20):  # Check bounds
            return False
...
for word in words:
    ...
    while time % 2 == 0 or condition:
        ...
        x = r.randint(0, 19)
        y = r.randint(0, 19)
        if word.orientation[0] * word.size < 19 - x and word.orientation[1] * word.size < 19 - y:
